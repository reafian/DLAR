...
function optout_file_checks {
	...
	echo "$(date "+%Y-%m-%d %H:%M:%S") - Check Optout files for consistency" >> $experian_report_file
	...
	echo "$(date "+%Y-%m-%d %H:%M:%S") - Optout file consistency check finished" >> $experian_report_file
}
